...
# Read the file and fix the curly braces
layout_path = r"C:\Users\user\.qclaw\astrology-clean\src\app\blog\layout.tsx"

with open(layout_path, "r", encoding="utf-8") as f:
    content = f.read()

# Fix the double curly braces - replace {{ with { and }} with }
# But we need to be careful with JSX expressions like {{ children }}
# Actually, in TypeScript/React, single { is for expressions
# Double {{ is not valid TS syntax - it should be single {
content = content.replace("{{", "{")
content = content.replace("}}", "}")

# But now JSX double braces for style objects will be wrong...
# Let me think again...

# Actually in the original file:
# import type {{ Metadata }} should be import type { Metadata }
# export const metadata: Metadata = {{ should be export const metadata: Metadata = {{
# 
# Wait, I'm confused. Let me look at valid TSX:
# - Single { is for expressions: {children}
# - Double {{ in TSX is for objects: style={{color: 'red'}}
# 
# But in the layout.tsx file, we have:
# import type {{ Metadata }} - this is WRONG, should be import type { Metadata }
# export const metadata: Metadata = {{ - this is WRONG, should be export const metadata: Metadata = {{
#
# Actually no - in the file content, if it shows {{, that's literal two braces
# In valid TSX, import statement uses single brace: import type { Metadata }
# And object literal uses single brace: metadata = {{
#
# So the problem is the file has DOUBLE braces where it should have SINGLE braces
# Except for JSX expressions where we need double braces for objects

# Let me rewrite the file correctly from scratch
...
print(f"Fixed layout.tsx")

# Now fix page.tsx
page_path = r"C:\Users\user\.qclaw\astrology-clean\src\app\blog\page.tsx"

with open(page_path, "r", encoding="utf-8") as f:
    content = f.read()

# The file has incorrect double braces - let me rewrite it completely
# I'll create the proper content
