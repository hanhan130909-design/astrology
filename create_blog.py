...
# Define the blog articles content
articles = [
    {
        "id": "1",
...
# Generate the blog page TypeScript file
blog_page_content = """'use client';
...
for i, article in enumerate(articles):
    blog_page_content += "  {\n"
    blog_page_content += f'    id: "{article["id"]}",\n'
    blog_page_content += f'    slug: "{article["slug"]}",\n'
    blog_page_content += f'    category: "{article["category"]}",\n'
    blog_page_content += f'    categoryZh: "{article["categoryZh"]}",\n'
    blog_page_content += f'    categoryEn: "{article["categoryEn"]}",\n'
    blog_page_content += f'    categoryId: "{article["categoryId"]}",\n'
    
    # Title
    blog_page_content += "    title: {\n"
    for lang in ["zh", "en", "id"]:
        blog_page_content += f'      {lang}: "{article["title"][lang]}",\n'
    blog_page_content += "    },\n"
    
    # Excerpt
    blog_page_content += "    excerpt: {\n"
    for lang in ["zh", "en", "id"]:
        blog_page_content += f'      {lang}: "{article["excerpt"][lang]}",\n'
    blog_page_content += "    },\n"
    
    # Content
    blog_page_content += "    content: {\n"
    for lang in ["zh", "en"]:
        content_escaped = article["content"][lang].replace('"', '\\"').replace('\n', '\\n')
        blog_page_content += f'      {lang}: "{content_escaped}",\n'
    blog_page_content += "    },\n"
    
    # Author and other fields
    blog_page_content += f'    author: "{article["author"]}",\n'
    blog_page_content += f'    authorEn: "{article["authorEn"]}",\n'
    blog_page_content += f'    authorId: "{article["authorId"]}",\n'
    blog_page_content += f'    date: "{article["date"]}",\n'
    blog_page_content += f'    readTime: {article["readTime"]},\n'
    
    # Tags
    blog_page_content += "    tags: ["
    blog_page_content += ", ".join([f'"{tag}"' for tag in article["tags"]])
    blog_page_content += "],\n"
...
# Write the blog page
output_path = r"C:\Users\user\.qclaw\astrology-clean\src\app\blog\page.tsx"
with open(output_path, "w", encoding="utf-8") as f:
    f.write(blog_page_content)

print(f"Created blog page: {output_path}")

# Now create the layout.tsx file
...
print(f"Created blog layout: {layout_path}")
print("Done! Both files created successfully.")
